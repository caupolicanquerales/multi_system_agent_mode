...
*With wrapper:*
```
$yml = '<YAML_BLOCK_PS_SINGLE_QUOTED>'; [System.IO.File]::WriteAllText("$env:TEMP\rewrite-custom.yml", $yml); echo 'Ensure Java 21 is set as the active JDK before running this command' ; & '<project_location>\mvnw.cmd' org.openrewrite.maven:rewrite-maven-plugin:6.44.0:run -B --no-transfer-progress "-Drewrite.configLocation=$env:TEMP\rewrite-custom.yml" -Drewrite.recipeArtifactCoordinates=<artifacts> "-Drewrite.activeRecipes=<recipes>,org.openrewrite.maven.UpgradeDependencyVersion,com.custom.openrewrite.MigrateLegacyDependencies" -Drewrite.options.org.openrewrite.maven.UpgradeDependencyVersion.groupId=* -Drewrite.options.org.openrewrite.maven.UpgradeDependencyVersion.artifactId=* -Drewrite.options.org.openrewrite.maven.UpgradeDependencyVersion.newVersion=LATEST
```

*Without wrapper:*
```
$yml = '<YAML_BLOCK_PS_SINGLE_QUOTED>'; [System.IO.File]::WriteAllText("$env:TEMP\rewrite-custom.yml", $yml); Set-Location '<project_location>' ; echo 'Ensure Java 21 is set as the active JDK before running this command' ; mvn org.openrewrite.maven:rewrite-maven-plugin:6.44.0:run -B --no-transfer-progress "-Drewrite.configLocation=$env:TEMP\rewrite-custom.yml" -Drewrite.recipeArtifactCoordinates=<artifacts> "-Drewrite.activeRecipes=<recipes>,org.openrewrite.maven.UpgradeDependencyVersion,com.custom.openrewrite.MigrateLegacyDependencies" -Drewrite.options.org.openrewrite.maven.UpgradeDependencyVersion.groupId=* -Drewrite.options.org.openrewrite.maven.UpgradeDependencyVersion.artifactId=* -Drewrite.options.org.openrewrite.maven.UpgradeDependencyVersion.newVersion=LATEST
```
